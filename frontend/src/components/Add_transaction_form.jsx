...
export default function TransactionForm({ userId, onSaveTransaction, onCloseForm, initialData = null, isEditing = false, isDarkTheme = true }) {
  ...
  return (
    <div>
      <h2 className={`text-xl font-bold mb-4 ${isDarkTheme ? 'text-gray-100' : 'text-gray-900'}`}>{isEditing ? t('editTransaction') : t('newTransaction')}</h2>
      <div className={`rounded-xl p-6 border ${isDarkTheme ? 'bg-gray-800/50 border-gray-700/50' : 'bg-white border-gray-300'}`}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-end pb-3 space-x-6">
              <label className={`inline-flex items-center cursor-pointer transition-colors ${isDarkTheme ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'}`}>
                ...
                />
                <span className="ml-2 text-sm">{t('incomeType')}</span>
              </label>
              <label className={`inline-flex items-center cursor-pointer transition-colors ${isDarkTheme ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'}`}>
                <input
                  type="radio"
                ...
                />
                <span className="ml-2 text-sm">{t('expenseType')}</span>
              </label>
            </div>
            <div>
              <label htmlFor="category" className={`block text-xs font-medium mb-1 uppercase tracking-wider ${isDarkTheme ? 'text-gray-400' : 'text-gray-600'}`}>{t('categoryLabel')}</label>
              <select id="category"
                value={category}
                ...
                required
              >
                <option value="" disabled hidden>{t('chooseCategory')}</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="date" className={`block text-xs font-medium mb-1 uppercase tracking-wider ${isDarkTheme ? 'text-gray-400' : 'text-gray-600'}`}>{t('dateLabel')}</label>
              <input
                type="date"
                id="date"
                value={date}
            ...
            <div>
              <label htmlFor="amount" className={`block text-xs font-medium mb-1 uppercase tracking-wider ${isDarkTheme ? 'text-gray-400' : 'text-gray-600'}`}>{t('amountLabel')}</label>
              <input
                type="number"
                id="amount"
                value={amount}
          ...
          <div>
            <label htmlFor="description" className={`block text-xs font-medium mb-1 uppercase tracking-wider ${isDarkTheme ? 'text-gray-400' : 'text-gray-600'}`}>{t('descriptionOptional')}</label>
            <textarea
              id="description"
              value={description}
